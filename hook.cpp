...
bool tailshook::call_kernel_function(void* kernel_function_address){
	...
	write_to_read_only(function, &orig, sizeof(orig));
...
NTSTATUS tailshook::hook_handler(PVOID called_param) {
	TAILS_MEMORY* instructions = (TAILS_MEMORY*)called_param;

	if (instructions->reqBase == TRUE) {
		ANSI_STRING AS;
		...
		if (instructions->address < 0x7FFFFFFFFFFF && instructions->address > 0) {
			...
			if (!memcpy(kernelBuff, instructions->bufferAddr, instructions->size)) {
				return STATUS_UNSUCCESSFUL;
			}
...
INT tailshook::FrameRect(HDC hDC, CONST RECT* lprc, HBRUSH hbr, int thickness)
{
	HBRUSH oldbrush;
	RECT r = *lprc;
